...
function formatRules (root, params) {
  ...
  root.walkRules(function (rule, index) {
    ...
    setRuleRawsConfig(rule, {
      ruleBefore: getRuleBefore(prev) + ruleBefore,
...
function setRuleRawsConfig (rule, opts) {
  var ruleWithoutBody = rule.ruleWithoutBody;

  rule.raws.before = opts.ruleBefore
  rule.raws.afterName = ' '
  setPropertyForAtRuleSemicolonNewlineAfter(rule.raws, opts, ruleWithoutBody)
  if (opts.stylelint['block-closing-brace-newline-after']) {
    setPropertyForBlockClosingBraceNewlineAfter(rule.raws, opts, ruleWithoutBody)
  } else {
    setPropertyForAtRuleSemicolonNewlineAfter(rule.raws, opts, ruleWithoutBody)
  }
...
function setPropertyForAtRuleSemicolonNewlineAfter (raws, opts, ruleWithoutBody) {
...
function setPropertyForBlockClosingBraceNewlineAfter (raws, opts, ruleWithoutBody) {
  raws.semicolon = true
